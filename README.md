...
Figstack is a Vite-based tool for building plugins for Figma. It focuses on one specific problem: making communication between the UI and the plugin code simple, type-safe, and pleasant to use.
...
There are libraries that improve this experience, including figwire. Figstack builds on that idea and integrates it directly into the build process so you don’t have to think about wiring at all.

## How it works

With Figstack, you write plugin-side functions normally and export them from files that follow a simple naming convention. During the build, Figstack detects those files, bundles them into a single plugin file, and automatically connects them to the UI using an internal figwire instance.
...
Figstack gives you a simple mental model: treat plugin-side functions as remote async functions. Everything is bundled into a single plugin file, fully typed, and easy to reason about.
